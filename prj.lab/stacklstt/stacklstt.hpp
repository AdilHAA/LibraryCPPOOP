...
template<class T>
    StackLstT<T>::StackLstT(StackLstT<T>&& x) noexcept {
        std::swap(head_, x.head_);
}
template<class T>
        StackLstT<T>& StackLstT<T>::operator=(StackLstT<T>&& x) noexcept {
        if (head_ != x.head_) {
            std::swap(head_, x.head_);
        }
        return *this;
...
template<class T>
    StackLstT<T>::StackLstT(const StackLstT<T>& x) {
        if (x.head_ != nullptr) {
            head_ = new Node;
            head_->v = x.head_->v;
            Node* new_head = head_;
...
template<class T>
    StackLstT<T>& StackLstT<T>::operator=(const StackLstT<T>& x) {
        if (x.head_ != head_) {
            if (x.head_ == nullptr) {
                while (head_ != nullptr) {
                    Pop();
